...
<template>
  <div class="flex items-center justify-center">
    <button class="flex flex-col items-center justify-center" @pointerdown="handleLeftPointerDown">
      <div class="w-8">
        <span class="font-mono text-xs font-bold">{{ counter }}</span>
      </div>
      <div
        class="size-8 border bg-neutral-200 border-neutral-600/30 data-[success=true]:bg-emerald-200 data-[failure=true]:bg-rose-200 transition-all duration-300 ease-in-out data-[success=true]:border-emerald-600/30 data-[failure=true]:border-rose-600/30 data-[locked=true]:bg-neutral-400 data-[locked=true]:border-neutral-800"
        :data-success="success" :data-failure="failure" :data-locked="locked">
        <span class="text-lg font-bold text-neutral-900">{{ string }}</span>
      </div>
    </button>
  </div>
</template>
